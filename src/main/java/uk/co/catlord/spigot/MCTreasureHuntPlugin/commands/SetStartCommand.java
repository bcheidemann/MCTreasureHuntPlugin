...
public class SetStartCommand extends RegisterableCommand {
  @Override
  ...
  public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
    ...
    Checkpoint checkpoint = new Checkpoint("START", options.location, null, new Point());
    Result<?, String> result = CheckpointDataStore.getStore().addCheckpoint(checkpoint);

    // Feedback to the player
    if (result.isError()) {
      sender.sendMessage(ChatColor.RED + "Failed to set start checkpoint: " + result.getError());
    } else {
      sender.sendMessage(ChatColor.GREEN + "Start checkpoint set successfully.");
    }
